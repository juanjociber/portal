...
$objPHPExcel->setActiveSheetIndex(0)
	...
	->setCellValue('T1', 'codigo')
	->setCellValue('U1', 'producto')
	->setCellValue('V1', 'cantidad')
	->setCellValue('W1', 'precio')
	->setCellValue('X1', 'medida');
...
// Agregar las lineas de la tabla Productos.
	try{
		...
		$stmt=$conmy->prepare("select c.id, c.cotizacion, c.fecha, c.cliruc, c.clinombre, c.clidireccion, c.clicontacto, c.clitelefono, c.clicorreo, c.vendnombre, c.tiempo, c.pago, c.entrega, c.lugar, 
		c.moneda, c.igv, c.descuento, c.obs, c.estado, d.codigo, d.producto, d.cantidad, d.precio, d.medida from tblcotizaciones c inner join tbldetallecotizacion d on c.id=d.idcotizacion limit 5000;");
		$stmt->execute();
		$i=1;
		if($stmt->rowCount()>0){
			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
				...
				$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('R'.$i, $row['obs'])
					->setCellValue('S'.$i, $Estado)
					->setCellValue('T'.$i, $row['codigo'])
					->setCellValue('U'.$i, $row['producto'])
					->setCellValue('V'.$i, $row['cantidad'])
					->setCellValue('W'.$i, $row['precio'])
					->setCellValue('X'.$i, $row['medida']);
