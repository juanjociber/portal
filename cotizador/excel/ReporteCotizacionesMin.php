...
$objPHPExcel->setActiveSheetIndex(0)
	...
	->setCellValue('K1', 'Tiempo Oferta')
	->setCellValue('L1', 'Forma Pago')
	->setCellValue('M1', 'Forma Entrega')
	->setCellValue('N1', 'Lugar Entrega')
	->setCellValue('O1', 'Moneda')
	->setCellValue('P1', 'IGV')
	->setCellValue('Q1', 'Descuento')
	->setCellValue('R1', 'Valor Venta')
	->setCellValue('S1', 'Total Descuentos')
	->setCellValue('T1', 'Base Imponible')
	->setCellValue('U1', 'Total Impuestos')
	->setCellValue('V1', 'Importe Total')
...
// Agregar las lineas de la tabla Productos.
	try{
		...
		$stmt=$conmy->prepare("select id, cotizacion, fecha, cliruc, clinombre, clidireccion, clicontacto, clitelefono, clicorreo, vendnombre, tiempo, pago, entrega, lugar, moneda, igv, descuento, tot_valorventa, tot_descuentos, base_imponible, tot_impuestos, tot_precioventa, obs, estado from tblcotizaciones limit 5000;");
		$stmt->execute();
		$i=1;
		if($stmt->rowCount()>0){
			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
				...
				$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('L'.$i, $row['pago'])
					->setCellValue('M'.$i, $row['entrega'])
					->setCellValue('N'.$i, $row['lugar'])
					->setCellValue('O'.$i, $row['moneda'])
					->setCellValue('P'.$i, $row['igv'])
					->setCellValue('Q'.$i, $row['descuento'])
					->setCellValue('R'.$i, $row['tot_valorventa'])
					->setCellValue('S'.$i, $row['tot_descuentos'])
					->setCellValue('T'.$i, $row['base_imponible'])
					->setCellValue('U'.$i, $row['tot_impuestos'])
					->setCellValue('V'.$i, $row['tot_precioventa'])
					->setCellValue('W'.$i, $row['obs'])
