async function fnAgregarCotizacion(){
    ...
    console.log(data);
    if (data.res === '200') {
        window.location.href='/portal/cotizador/admin/ResumenCotizacion.php?cotizacion='+data.cot;
    } else {
        alert(data.msg);
    }
...
async function fnAgregarCotizacion2(){
    ...
    const response = await this.fetch('/portal/cotizador/admin/insert/AgregarCotizacion.php', {
        method:'POST', 
        body:data
    })
    .then(res=>res.json())
