<?php
	...
	if($bandera){
		...
		if(!empty($_POST['producto'])){
			$query=" where concat(codinterno, nombre) like '%".$_POST['producto']."%'";
		}
		...
		try{
			...
			$stmt=$conmy->prepare("select id, codinterno, nombre, marca, medida, stock, pvpublico, pvmayor, pvflota, moneda, fecha, observacion, estado from tblcatalogo".$query." order by id desc limit :Page, 20;");
