...
if(!empty($_GET['cliente'])){
	$query = " where concat(ruc, nombre) like '%".$_GET['cliente']."%'";
}else{
	$query = " where fecha between '".$_GET['fechainicial']."' and '".$_GET['fechafinal']."'";
}

try{
	$conmy->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$stmt=$conmy->prepare("select id, numero, ruc, nombre, direccion, contacto, telefono, correo, observaciones, fecha, usuario, estado from tblclientes".$query.";");
	$stmt->execute();
	$i=1;
	if($stmt->rowCount()>0){
		while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
			...
			$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('F'.$i, $row['contacto'])
				->setCellValue('G'.$i, $row['telefono'])
				->setCellValue('H'.$i, $row['correo'])
				->setCellValue('I'.$i, $row['observaciones'])
				->setCellValue('J'.$i, PHPExcel_Shared_Date::PHPToExcel(date("d/m/Y", strtotime($row['fecha']))))
				->setCellValue('K'.$i, $row['usuario'])
				->setCellValue('L'.$i, $Estado);
		};
		$objPHPExcel->getActiveSheet()->getStyle('J2:J'.$i)->getNumberFormat()->setFormatCode("dd/mm/yyyy");
	}else{
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A2', "No se encontro resultados.");
	}
	$stmt=null;
