...
async function fnBuscarClientes2(cliente){
    ...
    const response = await this.fetch('search/BuscarClientes.php', {
        method:'POST', 
        body:data
        })
    .then(res=>res.json())
...
async function fnActualizarCliente2(){
    ...
    const response = await this.fetch('update/ActualizarCotizacionCliente.php', {
        method:'POST', 
        body:data
    })
    .then(res=>res.json())
