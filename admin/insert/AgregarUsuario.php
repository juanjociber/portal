...
if(!empty($_POST['role']) and !empty($_POST['nombre']) and !empty($_POST['usuario']) and !empty($_POST['clave']) and !empty($_SESSION['vgrole']) and !empty($_SESSION['vgusuario'])){
	if($_SESSION['vgrole']=='admin'){			
		if (fnValidarClave($_POST['clave'], $mensaje)){
			...
			try{
				...
				$stmt=$conmy->prepare("insert into tblusuarios(nombre, usuario, clave, role, creacion, actualizacion) values(:Nombre, :Usuario, :Clave, :Role, :Creacion, :Actualizacion);");
				$stmt->execute(array(
					'Nombre'=>$_POST['nombre'],
				...
				if($stmt->rowCount()>0){
					$res='200';
					$msg="BD: Se agregó correctamente el Usuario."; 
				}
				$stmt=null;
			}catch(PDOException $e){
				$msg='BD: Error al guardar la información.';
				$stmt=null;
			}
		}else{
			$msg=$mensaje;
