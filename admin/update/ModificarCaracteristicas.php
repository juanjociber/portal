<?php
    ...
    if(!empty($_SESSION['vgrole']) and !empty($_SESSION['vgusuario']) and !empty($_POST['id']) and !empty($_POST['caracteristicas'])){
        if($_SESSION['vgrole']=='admin'){
            $usuario=date('Ymd-His').'('.$_SESSION['vgusuario'].')';
            try{
                $conmy->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stmt=$conmy->prepare("update tblproductos set caracteristicas=:Caracteristicas, actualizacion=:Actualizacion where idproducto=:IdProducto;");
                $stmt->execute(array(
                    'IdProducto'=>$_POST['id'],
